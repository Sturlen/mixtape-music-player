...
describe("GET /api/playlists", () => {
  ...
  test("returns seeded playlists", async () => {
    ...
    ctx.playlistStore.tracks.set("t1", { id: "t1", name: "Song A", albumId: "a1", playtimeSeconds: 180, path: "/dev/null/a.mp3" } as any)
    ctx.playlistStore.tracks.set("t2", { id: "t2", name: "Song B", albumId: "a1", playtimeSeconds: 200, path: "/dev/null/b.mp3" } as any)
    const res = await app.handle(new Request("http://localhost/api/playlists"))
    const body = await res.json()
...
describe("GET /api/playlists/:id", () => {
  test("returns playlist by id", async () => {
    ...
    ctx.playlistStore.tracks.set("t1", { id: "t1", name: "Track X", albumId: "a1", playtimeSeconds: 180, path: "/dev/null/x.mp3" } as any)
    const res = await app.handle(new Request("http://localhost/api/playlists/p1"))
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  test("returns 404 for unknown playlist", async () => {
    const { app } = await createTestApp()
    const res = await app.handle(new Request("http://localhost/api/playlists/unknown"))
